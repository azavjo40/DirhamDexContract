...
describe("Exchange Contract", function () {
  ...
  describe("Deployment", function () {
    it("Should set the right owner", async function () {
      expect(await exchange.hasRole(await dirham.DEFAULT_ADMIN_ROLE(), owner.getAddress())).to.be.true;
    });
    ...
    it("Should set the right marketing role", async function () {
      expect(await exchange.hasRole(MARKETING_ROLE, owner.getAddress())).to.be.true;
    });
  });
  ...
  describe("Buy token", function () {
    it("Should allow buying tokens", async function () {
      ...
      const tx = await exchange.connect(owner).buyTokens(USDT_ADDRESS!, amountToBuy);
...
// npx hardhat test test/Exchange.test.ts --network sepolia
